...
tar_gz_file_path="/home/g800336/Desktop/Source /LOGS/322270039358_2024-07-02T02_55_51_device-logs_1719910551.tar.gz"
...
#         if log['trace'] == trace :
#             x=log['message']  
#             #cleaned_log = re.sub(r"\[.*?\]", "", log).strip() # Supprimer les parties entre crochets
#             pattern = re.compile(r'(?P<timestamp>\d{6,}\.\d{3})?\s*(?P<level>\w+):?\s*(?P<code>0x[\da-f]+)?\s*(?P<module>.*?\:)?\s*(?P<message>.*)', re.IGNORECASE)# Expression régulière pour extraire les champs
...
#                 message = match.group('message')
#             else:
#                 timestamp, level, code, module, message = None, None, None, None, x  

#             print("Timestamp: {}, Level: {}, Code: {}, Module: {}, Message: {}".format(timestamp,level,code,module,message))
#     print("le nombre des logs de type {} est {} ".format(trace, i))
...
def Dezip_file_tar_gz_statistiques(tar_gz_file_path) :
    ...
    if choix == 2 :
        ...
        while c not in range(1,5): 
            c = input("vous devez entrer un choix validé, reessayez : ")
        if c == 1 : 
            trace = 'err'
        elif c == 2 :
            trace = 'alert'
